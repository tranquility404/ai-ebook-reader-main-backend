...
public class MLTaskController {
    ...
    public MLTaskController(SummaryService summaryService, QuizService quizService, BookService bookService,
                            CloudinaryService cloudinaryService, EbookAnalysis ebookAnalysis,
                            AIService aiService) {
...
//        String healthRes = aiService.healthCheck();
...
//    }
    ...
    public ResponseEntity<?> generateSummary(@RequestBody GenerateDataRequest request) {
        ...
        if (summary != null) {   // Already generated
            ...
            return ResponseEntity.ok(cloudinaryService.getFileContent(summary.getResourceId()));
        }
...
//        Not generated yet, generating now...
        try {
            ...
            try (InputStream is = cloudinaryService.downloadFile(info.getCloudUrl())) {
                ...
                String generatedSummary = aiService.generateSummary(chapter.getTexts()); // Generate Summary
                String resourceId = cloudinaryService.uploadSummaryTxtToCloud(generatedSummary); // Save summary to cloud
...
//        Not generated yet, generating now...
        try {
            ...
            String summaryContent = cloudinaryService.getFileContent(summary.getResourceId());
            if (summaryContent.split(" ").length > 2000) throw new RuntimeException("Token limit exceeded");

            String generatedQuiz = aiService.generateQuiz(summaryContent); // Generate Quiz
            String resourceId = cloudinaryService.uploadQuizJsonToCloud(generatedQuiz); // Save quiz to cloud
    ...
    public ResponseEntity<?> getQuiz(@PathVariable String quizId) {
        ...
        return ResponseEntity.ok(cloudinaryService.getFileContent(quiz.getResourceId()));
    }
